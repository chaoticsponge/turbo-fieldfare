...
from agent_models import ROLES, catalog as role_catalog, model_path, route_config
...
def configure_fleet(state, roles, model_root, configured, launch, concurrency, context, worker_precision='8bit',
                    stream_profiles=None, expert_chunk_rows=16):
...
def main():
    ...
    parser.add_argument("--context", type=int,
                        choices=[4096, 8192, 16384, 32768, 65536], default=65536)
    parser.add_argument("--prefix-cache-gb", type=int, choices=[0, 2, 4, 8, 16], default=0,
                        help="Optional bounded SSD prefix reuse for repeated agent calls; 0 disables it")
    ...
    try:
        ...
        with (state_root / "server.lock").open("a") as lock:
            ...
            cache_key = (hashlib.sha256(''.join(data['revision'] for _, data in packages).encode()).hexdigest()
                         if args.fleet else catalog['revision'])
            ...
            if args.fleet:
                launch = configure_fleet(state, roles, model_root, configured, launch,
                                         ...
                                         stream_profiles, args.expert_chunk_rows)
